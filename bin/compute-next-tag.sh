...
#
# Tags look like `v<Collabora Online version>-<release>`:
#
# - if defaults/main.yml points at a Collabora Online version that has never
#   been released, the release counter restarts at 0 (`v26.04.3.1.1-0`)
# - otherwise the counter is incremented (`v26.04.3.1.1-1`), but only if
...
# `collabora_online_version` is the literal that Renovate rewrites (it carries
# the `# renovate:` annotation). The container image variables next to it are
# Jinja expressions built on top of it, so reading any of those would yield a
# template string rather than a version.
version="$(sed -nE 's|^collabora_online_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Collabora Online version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `26.04.3.1.1`), but the
# tags do (`v26.04.3.1.1-0`). Stripping any `v` before prepending one keeps
