...
**Note**: hosting Collabora Online under a subpath (by configuring the `docmost_path_prefix` variable) does not seem to be possible due to Collabora Online's technical limitations.
...
For a complete list of Collabora Online's config options that you could put in `docmost_environment_variables_additional_variables`, see its [environment variables](https://docmost.com/docs/self-hosting/environment-variables).
